...
(function(root, factory) {
	...
	.constant('gridsterConfig', {
		...
		rowHeight: '65', // height of grid rows. 'match' will make it the same as the column width, a numeric value will be interpreted as pixels, '/2' is half the column width, '*5' is five times the column width, etc.
		margins: [20, 20], // margins in between grid items
		outerMargin: true,
		isMobile: false, // toggle mobile view
		mobileBreakPoint: 950, // width threshold to toggle mobile mode
	...
	.controller('GridsterItemCtrl', function() {
		...
		this.init = function($element, gridster, itemId) {
		...
		function($document, $window, GridsterTouch) {
			function GridsterDraggable($el, scope, gridster, item, itemOptions) {
				...
				function dragStop(event) {
					...
					if(scope.saveWidgetPositions) scope.saveWidgetPositions(gridster);
					scope.$apply(function() {
		...
		function($parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
			return {
				...
				link: function(scope, $el, attrs, controllers) {
					...
					item.init($el, gridster, scope.__itemID);
