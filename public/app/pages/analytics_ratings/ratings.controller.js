 ...
 * Ratings controller
 */
ezondapp.controller('analyticsRatingsController', function($scope, $state, $cookies, $http, $q, userService, dashboardService,appDataService, integrationProviders, ratinggroupProviders, integrationService) {
    ...
    $scope.searchData = function() {
        ...
        if($scope.selectedFilter.length > 0){
            filter_value = $scope.selectedFilter[0];
            for(i in $scope.ratingsInfo.filters[$scope.selectdIndex][0]) {
                if($scope.ratingsInfo.filters[$scope.selectdIndex][0][i] == filter_value) 
                    filter_index = i;
            }            
        }
        filter_index++;
        post_network_id = $scope.ratingsInfo.networkID * 1 + 100 * $scope.selectdIndex + 10000 * $scope.ratingsInfo.id + 1000000 * filter_index;

       $http.post("/user/getDedicatedPage", {filter: filter_value, dashboardID: dashboardService.dashboard.id, startDate: $scope.grab_startdate, endDate: $scope.grab_enddate, networkID: post_network_id}).then(function(response) {
    ...
    $scope.refreshMetricSelected = function(firstload = false) {
        ...
        // console.log(typeof $cookies.get('selectedMetricsForAnalytics'))
        if (firstload && typeof $cookies.get('selectedMetricsForAnalytics'+$scope.selectdIndex+$state.params.kind) != 'undefined'){
            $scope.savedMetrics = JSON.parse($cookies.get("selectedMetricsForAnalytics"+$scope.selectdIndex+$state.params.kind))
            for(i=0; i< $scope.metricDatas.length; i++) {
        ...
        $cookies.putObject('selectedMetricsForAnalytics'+$scope.selectdIndex+$state.params.kind, $scope.selectMetricArray, {'expires': new Date(2020, 1, 1) })
