 ...
 * Ratings controller
 */
ezondapp.controller('fbadRatingsController', function($scope, $state, $cookies, $http, $q, userService, dashboardService, appDataService, integrationProviders, ratinggroupProviders, integrationService) {
    ...
    $scope.refreshMetricSelected = function(firstload = false) {
        ...
        if (firstload && typeof $cookies.get('selectedMetricsForFbad'+$scope.selectdIndex+$state.params.kind) != 'undefined'){
            $scope.savedMetrics = JSON.parse($cookies.get("selectedMetricsForFbad"+$scope.selectdIndex+$state.params.kind))
            for(i=0; i< $scope.metricDatas.length; i++) {
        ...
        $cookies.putObject('selectedMetricsForFbad'+$scope.selectdIndex+$state.params.kind, $scope.selectMetricArray, {'expires': new Date(2020, 1, 1)})
