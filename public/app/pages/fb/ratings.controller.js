 ...
 * Ratings controller
 */
ezondapp.controller('fbRatingsController', function($scope, $http, $state, $cookies, $q, userService, dashboardService, integrationProviders,appDataService, ratinggroupProviders, integrationService) {
    // If not logged in
    ...
    $scope.searchData = function() {
        ...
        setTimeout('$(".panel").addClass("loading");$(".common-widgets-table").addClass("loading");', 10);
    ...
    $scope.refreshMetricSelected = function(firstload = false) {
        ...
        if (firstload && typeof $cookies.get('selectedMetricsForFb'+$state.params.kind) != 'undefined'){
            console.log("firstload")
            $scope.savedMetrics = JSON.parse($cookies.get("selectedMetricsForFb"+$state.params.kind))
            for(i=0; i< $scope.ratingsInfo.metric.length; i++) {
        ...
        $cookies.putObject('selectedMetricsForFb'+$state.params.kind, $scope.selectMetricArray, {'expires': new Date(2020, 1, 1)})
