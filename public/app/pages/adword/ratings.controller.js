 ...
 * Ratings controller
 */
ezondapp.controller('adwordRatingsController', function($scope, $http, $cookies, $state, $q, userService, dashboardService, integrationProviders, appDataService, ratinggroupProviders, integrationService) {
    
    // If not logged in
    if ( !userService.user ) return;

    $scope.ratingsInfo = ratinggroupProviders.ratingsInfo;
    console.log(ratinggroupProviders.ratingsInfo.iconsAliases)
    $scope.icons = ratinggroupProviders.ratingsInfo.iconsAliases;
    ...
    $scope.refreshMetricSelected = function(firstload = false) {
        ...
        if (firstload && typeof $cookies.get('selectedMetricsForAdwords'+$scope.selectdIndex+$state.params.kind) != 'undefined'){
            $scope.savedMetrics = JSON.parse($cookies.get("selectedMetricsForAdwords"+$scope.selectdIndex+$state.params.kind))
            for(i=0; i< $scope.metricDatas.length; i++) {
        ...
        $cookies.putObject('selectedMetricsForAdwords'+$scope.selectdIndex+$state.params.kind, $scope.selectMetricArray, {'expires': new Date(2020, 1, 1)})
